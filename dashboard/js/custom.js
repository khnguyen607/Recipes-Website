...
fetch('layout.html')
    ...
    .then(data => {
        ...
        document.querySelector('.footer_iner.text-center').innerHTML = `<p>2023 © MyRecipes - Thiết kế bởi <a href="#"> Nhóm 2 </a></p>`

        if (window.location.href.includes('index.html')) document.getElementById('sidebar__index').classList.add('mm-active')
        else if (window.location.href.includes('recipes.html')) document.getElementById('sidebar__recipes').classList.add('mm-active')
...
document.addEventListener('DOMContentLoaded', function () {
    if (window.location.href.includes("user.html")) {
        fetch('../backend/get_data.php?table=user__recipes_all')
            ...
            .then(data => {
                DivtoClone = document.querySelector('.messages_list .list-group-item').cloneNode(true)
                document.querySelector('.messages_list .list-group-item').remove()

                data.forEach(item => {
                    var cloneDiv = DivtoClone.cloneNode(true);
                    
                    cloneDiv.setAttribute('id', 'user_' + item.id)
                    cloneDiv.querySelector("h4").textContent = item.name
                    cloneDiv.querySelector("p").textContent = item.email
                    cloneDiv.querySelector("span").textContent = item.recipes_quantity + " công thức"
                    document.querySelector('.messages_list .list-group').appendChild(cloneDiv);
                })
            })
    }
});
